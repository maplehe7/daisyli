...
<section class="testimonial-feature"><div class="wrap"><div class="quote-mark" aria-hidden="true">“</div><blockquote>${(siteLanguage==='zh'?chineseContent.testimonials:testimonials)[1].quote}</blockquote><p class="quote-person">Nicole Z. <span>· Client testimonial</span></p><a class="text-link" href="/testimonials">Testimonials ${arrow}</a></div></section>
${DaisyVideos.render()}${cta()}`;}
function communityCard(p){return `<a class="community-card" href="/neighborhoods/${p.slug}"><div><img src="https://maplehe7.github.io/daisyli/web/assets/${encodeURIComponent(p.image)}" alt="${p.name}, California" loading="lazy" width="500" height="560"></div><h3>${p.name} ${diagonal}</h3></a>`;}
...
function route(){cancelIdxRequests();DaisyMap.clear();DaisyProperty.close();const requestedLanguage=new URLSearchParams(location.search).get('lang');if(['en','zh'].includes(requestedLanguage))siteLanguage=requestedLanguage;const path=location.pathname.replace(/\/$/,'')||'/';history.scrollRestoration=path==='/'?'manual':'auto';const parts=path.split('/');shown=12;document.body.classList.toggle('home-page',path==='/');const renderers={'/':home,'/about':about,'/search':searchPage,'/featured':()=>catalogue('featured'),'/sold':()=>catalogue('sold'),'/contact':contact,'/testimonials':clientStories,'/account':DaisyAccount.render};main.innerHTML=renderers[path]?renderers[path]():parts[1]==='neighborhoods'?communities(parts[2]):parts[1]==='property'?detail(parts[2]):notFound();document.title=(path==='/'?'Orange County Real Estate':(main.querySelector('h1')?.textContent||'Daisy Li'))+' · Daisy Li';document.querySelectorAll('#main-nav a').forEach(a=>{if(a.pathname===path)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});bindPage(path);DaisyAccount.bind();if(parts[1]==='property')DaisyProperty.bind(parts[2]);bindMobileLayout();DaisyVideos.bind();applyLanguage();if(path==='/'&&!location.hash)window.scrollTo({top:0,left:0,behavior:'instant'});if(path==='/')loadLiveHighlights(document.getElementById('home-live-listings'));const communityGrid=document.getElementById('community-live-listings');if(communityGrid)loadLiveHighlights(communityGrid,communityGrid.dataset.city);}
